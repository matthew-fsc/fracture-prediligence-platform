...
| B4 | `mgmt_qualified` / `mgmt_total_functions` | 20% | How many of the company's core business functions (sales, delivery, finance, operations) have a qualified manager who could run that function without the owner? | Ratio: qualified ÷ total functions | ≥75% coverage → 90 pts; ≥50% → 60 pts; <50% → 15 pts |
