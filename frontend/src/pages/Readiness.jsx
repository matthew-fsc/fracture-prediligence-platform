...
import { ChevronDown, ChevronRight, Edit2, Check, X, Info, Shield, AlertTriangle, Users, HelpCircle, Grid3x3 } from 'lucide-react'
import {
...
function clientDrsTierInfo(score) {
...
function ClientReadinessView() {
  ...
  const tier = clientDrsTierInfo(drs)

  const categories = scores?.category_scores
    ? Object.entries(CLIENT_CATEGORY_META).map(([key, meta]) => {
        const raw = scores.category_scores[key]
        return { key, ...meta, score: typeof raw === 'object' ? raw.score : raw }
      })
    : []
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
            ...
            {isPending ? (
              ...
              <>
                <div className="flex items-end gap-2">
                  <span className={cn('text-7xl font-bold tabular-nums leading-none', tier?.color ?? 'text-foreground')}>{Math.round(drs)}</span>
                  <span className="text-2xl text-muted-foreground mb-2">/100</span>
                </div>
                {(drsLow != null && drsHigh != null) && <p className="text-xs text-muted-foreground mt-1">Range: {Math.round(drsLow)}–{Math.round(drsHigh)}</p>}
              </>
            ) : (
            ...
            <div className="flex-1 min-w-0" style={{ height: 200 }}>
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart data={radarData} margin={{ top: 8, right: 16, bottom: 8, left: 16 }}>
                  ...
                  <Tooltip contentStyle={{ background: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', borderRadius: 6, fontSize: 11 }} formatter={(v) => [`${v}/100`, 'Score']} />
                </RadarChart>
              </ResponsiveContainer>
            </div>
          )}
      ...
      <div className="bg-card rounded-xl border border-border p-5">
        <div className="flex items-center gap-2 mb-3"><Info className="w-4 h-4 text-muted-foreground" /><h3 className="text-sm font-semibold text-card-foreground">Understanding Your Score</h3></div>
        <p className="text-sm text-muted-foreground leading-relaxed">The Diligence Readiness Score (DRS) measures how well your business will hold up under a professional buyer's due-diligence process. A higher score typically means a cleaner transaction, fewer re-trades, and stronger multiple.</p>
      </div>
      <div>
        <p className="text-[11px] font-semibold text-muted-foreground uppercase tracking-widest mb-3">Score Breakdown</p>
        <div className="space-y-3">
          {isPending
            ? Array.from({ length: 6 }).map((_, i) => <Skeleton key={i} className="h-16" />)
            : categories.map(({ key, label, weight, icon, score }) => {
                const s = clientScoreStyle(score)
                return (
                  <div key={key} className="bg-card rounded-xl border border-border p-4">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-muted-foreground border border-border rounded px-1.5 py-0.5">{weight}% weight</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className={cn('text-xs font-medium', s.color)}>{s.tier}</span>
                        <span className={cn('text-sm font-bold tabular-nums', s.color)}>{score != null ? `${Math.round(score)}/100` : '—'}</span>
                      </div>
                    </div>
                    <div className="h-2 bg-muted rounded-full overflow-hidden">
                      <div className={cn('h-full rounded-full transition-all duration-700', s.bar)} style={{ width: `${Math.min(100, score ?? 0)}%` }} />
                    </div>
                  </div>
                )
              })}
...
export default function Readiness() {
  ...
  if (isClient) return <ClientReadinessView />
  return <AdvisorReadiness />
}

function AdvisorReadiness() {
  const companyId = useCompanyId()
