...
import { Target, ChevronDown, ChevronRight, AlertTriangle, ArrowRight, TrendingUp, Info, BookOpen, CheckCircle, Clock, Circle, Plus, Trash2, HelpCircle, Zap, DollarSign } from 'lucide-react'
...
function ClientValueGapView() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <SectionHeader icon={<Target className="w-5 h-5" />} title="Value Roadmap" subtitle="Targeted initiatives your advisor recommends to increase your enterprise value before going to market." />
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-card rounded-xl border border-border p-5">
          ...
          {isPending ? <Skeleton className="h-8 w-24" /> : currentEv != null ? <p className="text-2xl font-bold text-primary tabular-nums">{fmtM(currentEv)}</p> : <p className="text-sm text-muted-foreground">Pending</p>}
        </div>
        <div className="bg-card rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-5">
          ...
          {isPending ? <Skeleton className="h-8 w-24" /> : potentialEv != null ? <p className="text-2xl font-bold text-emerald-400 tabular-nums">{fmtM(potentialEv)}</p> : <p className="text-sm text-muted-foreground">Pending</p>}
        </div>
        <div className="bg-card rounded-xl border border-amber-500/20 bg-amber-500/5 p-5">
          ...
          {isPending ? <Skeleton className="h-8 w-24" /> : gap != null ? <p className="text-2xl font-bold text-amber-400 tabular-nums">{fmtM(gap)}</p> : <p className="text-sm text-muted-foreground">Pending</p>}
        </div>
      </div>
      <div>
        <p className="text-[11px] font-semibold text-muted-foreground uppercase tracking-widest mb-3">Recommended Initiatives ({initiatives.length})</p>
        {isPending && <div className="space-y-3">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-24" />)}</div>}
        {!isPending && initiatives.length === 0 && (
          <div className="bg-card rounded-xl border border-border p-8 text-center">
            <Target className="w-8 h-8 text-muted-foreground mx-auto mb-3" />
            <p className="text-sm text-muted-foreground">Your advisor hasn't added specific initiatives yet. Check back soon.</p>
          </div>
        )}
        {!isPending && initiatives.length > 0 && (
          <div className="space-y-3">
            {initiatives.map((init, idx) => (
              <div key={init.id ?? idx} className="bg-card rounded-xl border border-border p-4">
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="flex items-start gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      {init.category && <p className="text-[11px] text-muted-foreground mt-0.5">{init.category}</p>}
                    </div>
                  </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {init.timeline && <span className="flex items-center gap-1 text-[11px] text-muted-foreground"><Clock className="w-3 h-3" />{init.timeline}</span>}
                  {init.effort && <span className={cn('text-[10px] font-medium px-2 py-0.5 rounded border', EFFORT_COLORS[init.effort] ?? 'text-muted-foreground bg-muted/30 border-border')}>{init.effort} effort</span>}
                  {init.ev_impact && <span className={cn('text-[10px] font-medium px-2 py-0.5 rounded border', EV_IMPACT_COLORS[init.ev_impact] ?? 'text-muted-foreground bg-muted/30 border-border')}>{init.ev_impact} EV impact</span>}
                  {init.cost_estimate != null && <span className="flex items-center gap-1 text-[11px] text-muted-foreground"><DollarSign className="w-3 h-3" />~{fmtM(init.cost_estimate)} cost</span>}
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="bg-card rounded-xl border border-border p-5">
        <div className="flex items-center gap-2 mb-3"><Info className="w-4 h-4 text-muted-foreground" /><h3 className="text-sm font-semibold text-card-foreground">How to Use This Roadmap</h3></div>
        <p className="text-sm text-muted-foreground leading-relaxed">Each initiative represents a specific operational or financial improvement that your advisor estimates will increase your enterprise value. Work through these with your advisor before going to market — completing high-impact items first typically yields the best return on time.</p>
      </div>
    </div>
  )
}
...
export default function ValueGap() {
  ...
  if (isClient) return <ClientValueGapView />
  return <AdvisorValueGap />
}

function AdvisorValueGap() {
