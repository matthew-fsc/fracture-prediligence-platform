...
// ── Client (owner) view — simplified read-only valuation ─────────────────
function ClientValuationView() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div style={{ height: 200 }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={barData} barSize={48} margin={{ top: 8, right: 8, bottom: 0, left: 8 }}>
                ...
                <YAxis tickFormatter={(v) => fmtM(v)} tick={{ fill: 'hsl(var(--muted-foreground))', fontSize: 10 }} axisLine={false} tickLine={false} width={52} />
                <Tooltip formatter={(v) => [fmtM(v), 'Enterprise Value']} contentStyle={{ background: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', borderRadius: 6, fontSize: 12 }} />
      ...
      {(ebitdaBase != null || multipleFloor != null) && (
        <div className="bg-card rounded-xl border border-border p-5">
          <p className="text-[11px] font-semibold text-muted-foreground uppercase tracking-widest mb-4">Key Inputs</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            ...
            {multipleFloor != null && multipleCeiling != null && (
              <div>
                ...
                <p className="text-base font-semibold text-card-foreground">{multipleFloor.toFixed(1)}× – {multipleCeiling.toFixed(1)}×</p>
              </div>
            )}
            {drs != null && (
              <div>
      ...
      <div className="bg-card rounded-xl border border-border p-5">
        ...
        <p className="text-sm text-muted-foreground leading-relaxed">
          Your enterprise value is calculated using a normalized EBITDA adjusted for owner add-backs, then
          multiplied by an EBITDA multiple range that reflects your Diligence Readiness Score and current
          market benchmarks for your industry. A higher DRS typically supports a higher multiple.
        </p>
...
export default function Valuation() {
  ...
  if (isClient) return <ClientValuationView />
  return <AdvisorValuation />
}

function AdvisorValuation() {
